...
bnd node_splitting(bnd &T){
		...
		for(int i=size/2+1;i<size+1;i++){
			...
			*temp->next[k] = *ab[i];
			...
			temp->next[k]->pptr=temp;
				...
				k++;
			T->next[i]=NULL;	
			//if(data==9&&c)cout<<temp->next[0]->data[0]<<" ";	
			//if(T->data[0]==12)cout<<temp->next[--k]->data[0];
		}
	...
	//T->data[size/2]=0;
		//if(data==9&&c)cout<<temp->next[1]->data[0]<<" ";
	data=a[size/2];
//	if(a[size/2]==9)cout<<T->data[1]<<"  ";
	tt=NULL;
